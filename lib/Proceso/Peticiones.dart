...
class peticiosDB {
  static Future<void> CrearTabla(sql.Database database) async {
    await database.execute(
        """"" CREATE TABLE Posiciones( id INTEGER PRIMARY KEY AUTOINCREMENT NULL, coordenadas, TEXT
    fecha, TEXT""");
  }

  static Future<sql.Database> db() async {
    return sql.openDatabase("minticGeodb", version: 1,
  ...
  static Future<List<Map<String, dynamic>>> MostrarTodasUbicaciones() async {
    final base = await peticiosDB.db();
  ...
  static Future<void> GuardarPosicion(coord, fec) async {
    final base = await peticiosDB.db();
    final datos = {"coordenadas": coord, "fecha": fec};
